...
[![codecov](https://codecov.io/gh/FlorianSchwieren/Test/branch/main/graph/badge.svg?token=Test_token_here)](https://codecov.io/gh/FlorianSchwieren/Test)
[![CI](https://github.com/FlorianSchwieren/Test/actions/workflows/main.yml/badge.svg)](https://github.com/FlorianSchwieren/Test/actions/workflows/main.yml)

Awesome test created by FlorianSchwieren

## Install

from source
```bash
git clone https://github.com/FlorianSchwieren/Test test
cd test
make install
```
...
```bash
❯ test create-user --help
Usage: test create-user [OPTIONS] USERNAME PASSWORD
...
```bash
test create-user admin admin --superuser
```
...
Run with `test run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `test create-user`.
...
test/app.py                  16      1    94%
test/cli.py                  21      0   100%
test/config.py                5      0   100%
test/db.py                   10      0   100%
test/models/__init__.py       0      0   100%
test/models/content.py       47      1    98%
test/routes/__init__.py      11      0   100%
test/routes/content.py       52     25    52%
test/routes/security.py      15      1    93%
test/routes/user.py          52     26    50%
test/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in test/default.toml.

### As environment variables
```bash
export test_KEY=value
export test_KEY="@int 42"
export test_KEY="@jinja {{ this.db.uri }}"
export test_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
